...
#define IS_METHOD(name, equals) strcmp(method, equals) == 0

struct _BiometricStoragePlugin {
  GObject parent_instance;
};

G_DEFINE_TYPE(BiometricStoragePlugin, biometric_storage_plugin,
              g_object_get_type())
...
static FlMethodResponse *_handle_error(const gchar *message, GError *error) {
  ...
  g_autofree gchar *error_message = g_strdup_printf(
      "%s: %s (%d) (%s)", message, error->message, error->code, domain);
  g_warning("%s", error_message);
  g_autoptr(FlValue) error_details = fl_value_new_map();
  fl_value_set_string_take(error_details, "domain",
                           fl_value_new_string(domain));
  fl_value_set_string_take(error_details, "code",
...
static FlMethodResponse *handleInit(FlValue *args) {
  ...
  FlValue *authRequired =
      fl_value_lookup_string(options, "authenticationRequired");
  if (fl_value_get_bool(authRequired)) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        kBadArgumentsError,
        "Linux plugin only supports non-authenticated secure storage",
        nullptr));
  }
...
const SecretSchema *biometric_get_schema(void) {
  static const SecretSchema the_schema = {
      "design.codeux.BiometricStorage",
      SECRET_SCHEMA_NONE,
      {
...
static void on_password_lookup(GObject *source, GAsyncResult *result,
                               gpointer user_data) {
  printf("on_password_lookup\n");
  GError *error = NULL;
  FlMethodCall *method_call = (FlMethodCall *)user_data;
  g_autoptr(FlMethodResponse) response = nullptr;
  printf("secret_password_lookup_finish\n");
...
static void
biometric_storage_plugin_class_init(BiometricStoragePluginClass *klass) {
...
void biometric_storage_plugin_register_with_registrar(
  ...
  g_autoptr(FlMethodChannel) channel =
      fl_method_channel_new(fl_plugin_registrar_get_messenger(registrar),
                            "biometric_storage", FL_METHOD_CODEC(codec));
