    ...
    return ExpansionTile(title: Row(
      ...
      CheckboxListTile(
          onChanged: (v) => widget.vm.callback(v
              ? (widget.vm.currentBlacklist..remove(HomeworkType.grade)..remove(HomeworkType.gradeGroup))
              : (widget.vm.currentBlacklist..add(HomeworkType.grade)..add(HomeworkType.gradeGroup))),
          title: Text("Noten & Tests"),
          value: !widget.vm.currentBlacklist.contains(HomeworkType.grade)),
      CheckboxListTile(
          onChanged: (v) => widget.vm.callback(v
              ? (widget.vm.currentBlacklist..remove(HomeworkType.homework)..remove(HomeworkType.lessonHomework))
              : (widget.vm.currentBlacklist..add(HomeworkType.homework)..add(HomeworkType.lessonHomework))),
